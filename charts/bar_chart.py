...
def get_win_ratio_per_hour(df: pd.DataFrame, time_classes) -> pd.DataFrame:
    ...
    for time_class in time_classes:
        ...
        dff.insert(len(dff.columns), time_class+' win ratio',
                   round(won_games/total_games, 2)*100)
        dff.insert(len(dff.columns), 'total '+time_class+' games', total_games)
    return dff


def render(app: Dash, df: pd.DataFrame, id=ids.BAR_CHART) -> html.Div:
    @app.callback(
        Output(id, 'children'),
        [Input(ids.YEAR_DROPDOWN, 'value'), Input(ids.TIME_CLASS_DROPDOWN, 'value')])
    def update_bar_chart(years: list, time_class) -> html.Div:
        dff = df.query('date.dt.year in @years')
        fig = px.bar(get_win_ratio_per_hour(dff, time_class),
        ...
        return html.Div(dcc.Graph(figure=fig), id=id, className='chart-bar')
    return html.Div(id=id, className='chart-bar')
